...
1. 来源锁定（最高优先级）
   ...
   - 引用标记、脚注号、方括号编号都属于正文的一部分，必须原样保留，例如 `DPP[2]`、`[2]`、`[2,3]`、`(2)`；禁止删除、漏掉、改写，禁止把这类标记从相邻文本中拆开
...
- 右侧必须100%来自译文，一字不改
- 禁止翻译、编造、补充任何内容
- 引用标记、脚注号、方括号编号必须原样保留，例如 `DPP[2]`、`[2]`、`[2,3]`、`(2)`；不要删除，不要把它们从相邻文本中拆开
...
- 禁止翻译、编造、补充任何内容
- 保留所有标点符号和序号
- 保留引用标记、脚注号、方括号编号，例如 `DPP[2]`、`[2]`、`[2,3]`、`(2)`；不要删除，也不要拆成独立碎片
...
def _coverage_ignorable_char(ch):
    """覆盖校验中视为空白的字符：普通空白 + Word 中常见不可见格式符。"""
    if ch == _COVERAGE_CONSUMED_MARK:
        return False
    return ch.isspace() or unicodedata.category(ch) == "Cf"


def _coverage_report_path(excel_path):
    base, _ = os.path.splitext(excel_path)
    return f"{base}_覆盖校验报告.xlsx"


def _coverage_cell_text(value):
    if value is None:
        return ""
    try:
        if pd.isna(value):
...
def _as_alignment_rows(data):
    """将 DataFrame 或列表统一成 [{'原文': ..., '译文': ...}]。"""
    ...
    for row in records:
        rows.append({
            "原文": _coverage_cell_text(row.get("原文", "")),
            "译文": _coverage_cell_text(row.get("译文", "")),
        })
    return rows


def _meaningful_len(text):
    return sum(1 for ch in str(text or "") if ch != _COVERAGE_CONSUMED_MARK and not _coverage_ignorable_char(ch))
...
def _coverage_match_status(match):
    if match.get("ok"):
        return "是"
    if match.get("mode") == "already_consumed":
        return "已被前序行消费"
    return "否"


class CoverageTextConsumer:
    """按顺序消费源文本，支持完全匹配和纯空白差异匹配。"""

    def __init__(self, text):
        self.original_chars = list(str(text or ""))
        self.chars = list(str(text or ""))

    def find_match(self, fragment):
        fragment = str(fragment or "").strip()
        if not fragment:
            return {"ok": True, "start": None, "end": None, "actual": "", "mode": "empty"}

        current = "".join(self.chars)
        exact_pos = current.find(fragment)
        if exact_pos >= 0:
            return self._build_match(exact_pos, exact_pos + len(fragment), "exact")

        needle = "".join(ch for ch in fragment if not _coverage_ignorable_char(ch))
        if not needle:
            return {"ok": False, "mode": "not_found"}

        norm_pos, index_map = self._find_normalized(self.chars, needle)
        if norm_pos >= 0:
            start = index_map[norm_pos]
            end = index_map[norm_pos + len(needle) - 1] + 1
            return self._build_match(start, end, "whitespace")

        full_pos, _ = self._find_normalized(self.original_chars, needle)
        if full_pos >= 0:
            return {"ok": False, "mode": "already_consumed"}

        return {"ok": False, "mode": "not_found"}

    def _find_normalized(self, chars, needle):
        hay_chars = []
        index_map = []
        for idx, ch in enumerate(chars):
            if ch == _COVERAGE_CONSUMED_MARK:
                hay_chars.append(_COVERAGE_CONSUMED_MARK)
                index_map.append(idx)
            elif not _coverage_ignorable_char(ch):
                hay_chars.append(ch)
                index_map.append(idx)

        hay = "".join(hay_chars)
        return hay.find(needle), index_map

    def _build_match(self, start, end, mode):
        actual = "".join(ch for ch in self.chars[start:end] if ch != _COVERAGE_CONSUMED_MARK)
        return {"ok": True, "start": start, "end": end, "actual": actual.strip(), "mode": mode}

    def consume(self, match):
        if not match.get("ok") or match.get("start") is None:
            return
        for idx in range(match["start"], match["end"]):
            self.chars[idx] = _COVERAGE_CONSUMED_MARK

    def remaining_text(self):
        text = "".join("\n" if ch == _COVERAGE_CONSUMED_MARK else ch for ch in self.chars)
        text = re.sub(r"\n{2,}", "\n", text)
        lines = [line.strip() for line in text.splitlines() if line.strip()]
        return "\n".join(lines)

    def remaining_len(self):
        return _meaningful_len(self.remaining_text())


def _consume_alignment_rows(rows, original_consumer, translated_consumer, phase, origin):
    accepted = []
    rejected = []

    for idx, row in enumerate(_as_alignment_rows(rows), 1):
        original = row.get("原文", "")
        translated = row.get("译文", "")
        ...
        original_match = original_consumer.find_match(original) if original else {"ok": True, "actual": "", "mode": "empty"}
        translated_match = translated_consumer.find_match(translated) if translated else {"ok": True, "actual": "", "mode": "empty"}

        if original_match.get("ok") and translated_match.get("ok"):
            if original:
                original_consumer.consume(original_match)
            if translated:
                translated_consumer.consume(translated_match)
            accepted.append({
                "原文": original_match.get("actual", ""),
                "译文": translated_match.get("actual", ""),
            })
            continue

        problems = []
        if original and not original_match.get("ok"):
            problems.append("原文已被前序行消费" if original_match.get("mode") == "already_consumed" else "原文未命中")
        if translated and not translated_match.get("ok"):
            problems.append("译文已被前序行消费" if translated_match.get("mode") == "already_consumed" else "译文未命中")

        rejected.append({
            "阶段": phase,
            "来源": origin,
            "原行号": idx,
            "问题类型": "、".join(problems) or "未命中",
            "原文命中": _coverage_match_status(original_match),
            "译文命中": _coverage_match_status(translated_match),
            "原文匹配模式": original_match.get("mode", ""),
            "译文匹配模式": translated_match.get("mode", ""),
            "原文": original,
            "译文": translated,
        })
...
def _save_coverage_report(report_path, rejected_rows=None, appended_rows=None,
                          remaining_original="", remaining_translated="", force=False):
    rejected_rows = rejected_rows or []
    appended_rows = appended_rows or []
    remaining_rows = [
        {
            "类型": "剩余原文",
            "非空白字符数": _meaningful_len(remaining_original),
            "内容摘要": _preview_text(remaining_original),
        },
        {
            "类型": "剩余译文",
            "非空白字符数": _meaningful_len(remaining_translated),
            "内容摘要": _preview_text(remaining_translated),
        },
    ]
    ...
    has_content = (
        rejected_rows
        or appended_rows
        or _meaningful_len(remaining_original) > 0
        or _meaningful_len(remaining_translated) > 0
    )
    if not report_path or (not force and not has_content):
        return
    ...
    try:
        rejected_df = pd.DataFrame(
            ...
            columns=[
                "阶段", "来源", "原行号", "问题类型",
                "原文命中", "译文命中", "原文匹配模式", "译文匹配模式",
                "原文", "译文",
            ],
        )
        appended_df = pd.DataFrame(
            appended_rows,
            columns=["阶段", "补漏轮次", "原文", "译文"],
        )
        remaining_df = pd.DataFrame(
            remaining_rows,
            columns=["类型", "非空白字符数", "内容摘要"],
        )

        with pd.ExcelWriter(report_path, engine="openpyxl") as writer:
            rejected_df.to_excel(writer, sheet_name="剔除行", index=False)
            appended_df.to_excel(writer, sheet_name="补漏追加行", index=False)
            remaining_df.to_excel(writer, sheet_name="剩余摘要", index=False)
        log_manager.log(f"覆盖校验报告已保存: {report_path}")
    except Exception as e:
...
def validate_alignment_coverage_only(data, original_text, translated_text, report_path=None, phase="分片覆盖校验"):
    ...
    original_consumer = CoverageTextConsumer(original_text)
    translated_consumer = CoverageTextConsumer(translated_text)

    accepted, rejected = _consume_alignment_rows(
        rows, original_consumer, translated_consumer, phase, "LLM结果"
    )

    remaining_original = original_consumer.remaining_text()
    remaining_translated = translated_consumer.remaining_text()

    if rejected:
        log_manager.log_exception(
            f"{phase}: 剔除 {len(rejected)} 行未命中源文件的结果",
            f"通过 {len(accepted)}/{len(rows)} 行"
        )
    else:
        log_manager.log(f"{phase}: {len(accepted)}/{len(rows)} 行通过覆盖校验")
    ...
    _save_coverage_report(
        ...
        remaining_translated=remaining_translated,
        force=bool(rejected),
    )
...
def _repair_alignment_rows_with_full_text(rows, original_text, translated_text, model_id,
                                          source_lang="中文", target_lang="英语",
                                          report_path=None,
                                          max_rounds=COVERAGE_REPAIR_MAX_ROUNDS):
    original_consumer = CoverageTextConsumer(original_text)
    translated_consumer = CoverageTextConsumer(translated_text)

    accepted, rejected = _consume_alignment_rows(
        rows, original_consumer, translated_consumer, "最终覆盖校验", "初始结果"
    )
    ...
    for round_idx in range(1, max_rounds + 1):
        ...
        if _meaningful_len(remaining_original) == 0 and _meaningful_len(remaining_translated) == 0:
            break

        log_manager.log(
            f"最终补漏第 {round_idx}/{max_rounds} 轮: "
            f"剩余原文 {original_consumer.remaining_len()} 字符，"
            f"剩余译文 {translated_consumer.remaining_len()} 字符"
        )

        anchor_hint = f"""
以下是覆盖校验后剩余的原文和译文片段。
请只对这些剩余片段进行对齐；如果某侧没有对应内容，按“原文 |||”或“||| 译文”输出。
"""
        system_prompt = get_docx_alignment_prompt(source_lang, target_lang)
        user_prompt = build_alignment_user_prompt(anchor_hint, remaining_original, remaining_translated)
        response = call_llm_stream(system_prompt, user_prompt, model_id, f"最终补漏第{round_idx}轮")

        if not response:
            log_manager.log_exception(f"最终补漏第 {round_idx} 轮 LLM 返回为空")
            break

        repair_candidates = parse_alignment_response(response)
        if not repair_candidates:
            log_manager.log_exception(f"最终补漏第 {round_idx} 轮未解析到有效行")
            break

        repaired, repair_rejected = _consume_alignment_rows(
            repair_candidates,
            original_consumer,
            translated_consumer,
            f"最终补漏第{round_idx}轮",
        ...
        all_rejected.extend(repair_rejected)
    ...
    final_remaining_original = original_consumer.remaining_text()
    final_remaining_translated = translated_consumer.remaining_text()
    ...
    if _meaningful_len(final_remaining_original) or _meaningful_len(final_remaining_translated):
        log_manager.log_exception(
            "最终补漏后仍有未覆盖内容",
            f"原文剩余 {original_consumer.remaining_len()}，译文剩余 {translated_consumer.remaining_len()}"
        )
    else:
        log_manager.log("最终覆盖校验完成：原文与译文均已覆盖")
...
def apply_final_coverage_repair_to_excel(excel_path, original_path, translated_path, model_id,
                                         source_lang="中文", target_lang="英语",
                                         max_rounds=COVERAGE_REPAIR_MAX_ROUNDS):
    """对已有最终 Excel 执行完整源文件覆盖校验和补漏。"""
    if not excel_path or not os.path.exists(excel_path):
        log_manager.log_exception("最终覆盖校验跳过：Excel 文件不存在", excel_path)
        return False
    ...
    if not original_text and not translated_text:
        log_manager.log_exception("最终覆盖校验跳过：源文件内容为空")
        return False

    try:
        df = pd.read_excel(excel_path)
    except Exception as e:
        log_manager.log_exception("最终覆盖校验读取 Excel 失败", str(e))
        return False
    ...
    final_rows, repair_info = _repair_alignment_rows_with_full_text(
        ...
        report_path=_coverage_report_path(excel_path),
...
"""
            ...
            coverage_original = read_file_content(file_original_path)
            coverage_trans = read_file_content(file_trans_path)
            all_data, _ = validate_alignment_coverage_only(
                all_data,
                coverage_original,
                coverage_trans,
...
def merge_and_deduplicate_excels(excel_paths, final_output_path, source_lang="中文", target_lang="英语",
                                 original_path=None, translated_path=None, model_id=None,
                                 enable_final_repair=True):
    ...
    if enable_final_repair and original_path and translated_path and model_id:
        ...
        if original_text or translated_text:
            repaired_rows, repair_info = _repair_alignment_rows_with_full_text(
                ...
                source_lang=source_lang,
                target_lang=target_lang,
                report_path=_coverage_report_path(final_output_path),
                max_rounds=COVERAGE_REPAIR_MAX_ROUNDS,
            )
            combined_df = pd.DataFrame(repaired_rows, columns=['原文', '译文']).reset_index(drop=True)
            log_manager.log(f"最终覆盖校验后行数: {len(combined_df)}")
        else:
            log_manager.log_exception("最终覆盖校验跳过：完整源文件内容为空")
    ...
    def run_processing(self):
        ...
        try:
            ...
            if not self.processing_stopped and not stop_following_tasks:
                ...
                if final_path and os.path.exists(final_path) and split_parts <= 1:
                    log_manager.log("「阶段四」最终覆盖校验与补漏")
                    apply_final_coverage_repair_to_excel(
                        final_path,
                        original_path,
                        trans_path,
                        model_id,
