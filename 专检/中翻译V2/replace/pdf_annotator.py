...
from zhongfanyi.llm.llm_project.replace.text_matcher import TextMatcher, generate_search_variants
...
class PDFAnnotator:
                ...
                if text_instances:
                    ...
                    if len(text_instances) == 1:
                        ...
                        for rect in text_instances:
                            y_key = round(rect.y0, 1)  # 使用y0坐标分组
                            if y_key not in lines:
                                lines[y_key] = []
                            lines[y_key].append(rect)
                        
                        # 对每一行的矩形进行合并
                        for y_key, rects in lines.items():
                            # 按x坐标排序
    ...
    def highlight_comment_and_replace(
        self,
        search_text: str,
        new_text: str,
        comment: str,
        ...
        """
        # 第一步：使用智能匹配添加批注
        annot_count, position_key = self.highlight_and_comment_with_context(
        ...
        if not instances:
            if debug:
                print(f"    [调试] 批注后未找到实例，跳过替换")
            return (annot_count, position_key)
        
        # 如果没有上下文或只有一个实例，使用第一个
        if not context or len(instances) == 1:
            instance = instances[0]
        else:
            # 使用上下文匹配找到最佳实例
            instance = self._find_best_match_with_context(
                instances, search_text, context, debug
            )
            if not instance:
                if debug:
                    print(f"    [调试] 上下文匹配失败，使用第一个实例")
                instance = instances[0]
        ...
        if success:
            if debug:
                print(f"    [调试] ✓ 替换成功: '{search_text}' → '{new_text}'")
            return (1, position_key)
        else:
            if debug:
                print(f"    [调试] ✗ 替换失败")
            return (0, position_key)
    ...
    def _find_best_match_with_context(
        ...
        def extract_context_parts(context_text, target_text):
            """提取目标文本前后的上下文"""
            ...
            idx = normalized_context.index(normalized_target)
            before = normalized_context[:idx]
            after = normalized_context[idx + len(normalized_target):]
        ...
        if clean_context == clean_search:
            if debug:
                print(f"    [调试] 上下文无效：只包含搜索文本")
            return None
        
        # 提取上下文的前后部分
        context_before, context_after = extract_context_parts(context, search_text)
        
        if debug:
            print(f"    [调试] 上下文分析:")
            print(f"           前文: {context_before[-30:] if len(context_before) > 30 else context_before}")
            print(f"           目标: {clean_search}")
            print(f"           后文: {context_after[:30] if len(context_after) > 30 else context_after}")
        ...
        for idx, instance in enumerate(instances):
            ...
            # 扩展矩形区域以获取上下文（增大窗口以获取更多上下文）
            expanded_rect = fitz.Rect(
                max(0, rect.x0 - 200),  # 增加到200
                max(0, rect.y0 - 30),
            ...
            # 提取实例周围的前后文
            instance_before, instance_after = extract_context_parts(surrounding_text, search_text)
            
            if debug:
                print(f"    [调试] 实例 {idx+1} (页面 {page_num+1}, x={rect.x0:.1f}):")
                print(f"           前文: {instance_before[-30:] if len(instance_before) > 30 else instance_before}")
                print(f"           后文: {instance_after[:30] if len(instance_after) > 30 else instance_after}")
            
            # === 策略1: 精确前后文匹配（最高优先级）===
            exact_before_match = context_before and instance_before and context_before in instance_before
            exact_after_match = context_after and instance_after and context_after in instance_after
            
            if exact_before_match and exact_after_match:
                # 完美匹配：前后文都精确匹配
                score = 1.0
                if debug:
                    print(f"           ✓ 完美匹配（前后文都精确）")
                if score > best_score:
                    best_score = score
                    best_match = instance
                    best_details = {'strategy': '完美匹配', 'before': True, 'after': True}
                continue
            elif exact_before_match or exact_after_match:
                # 部分精确匹配
                score = 0.85
                if debug:
                    print(f"           ✓ 部分精确匹配（{'前文' if exact_before_match else '后文'}）")
                if score > best_score:
                    best_score = score
                    best_match = instance
                    best_details = {'strategy': '部分精确匹配', 'before': exact_before_match, 'after': exact_after_match}
                continue
            
            # === 策略2: 最长公共子串匹配 ===
            lcs_before = longest_common_substring_length(context_before, instance_before)
            lcs_after = longest_common_substring_length(context_after, instance_after)
            
            lcs_before_score = lcs_before / len(context_before) if context_before else 0
            lcs_after_score = lcs_after / len(context_after) if context_after else 0
            
            # === 策略3: 字符重叠度 ===
            char_before_score = sum(1 for c in context_before if c in instance_before) / len(context_before) if context_before else 0
            char_after_score = sum(1 for c in context_after if c in instance_after) / len(context_after) if context_after else 0
            
            # === 综合评分（前后文平衡）===
            # 权重：LCS 60%, 字符重叠 40%
            before_score = lcs_before_score * 0.6 + char_before_score * 0.4
            after_score = lcs_after_score * 0.6 + char_after_score * 0.4
            
            # 前后文平均分（如果只有一侧有上下文，则只用该侧）
            ...
            if debug:
                print(f"           得分: {score:.2%} (前文: {before_score:.2%}, 后文: {after_score:.2%})")
            ...
            if score > best_score:
                ...
                best_details = {
                    ...
                    'lcs_before': lcs_before_score,
                    'lcs_after': lcs_after_score
                }
        
        if debug:
            print(f"    [调试] 最佳匹配: 得分 {best_score:.2%}, 策略: {best_details.get('strategy', 'N/A')}")
        
        # 要求至少50%的匹配度（对于综合匹配）
        # 但精确匹配和部分精确匹配已经有更高的分数
        if best_match and best_score >= 0.5:
            return best_match
        
        if debug:
            print(f"    [调试] ❌ 所有实例得分都低于阈值 (最高: {best_score:.2%})")
    ...
    def highlight_and_comment_with_context(
        ...
        # 有多个实例且提供了上下文，使用增强的上下文匹配
        if debug:
            print(f"    [调试] 多个实例，使用增强上下文匹配")
        
        best_match = self._find_best_match_with_context(
            instances, search_text, context, debug
        )
