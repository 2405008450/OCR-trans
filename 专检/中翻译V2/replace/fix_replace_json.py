...
def is_list_pattern(s: str) -> bool:
    """判断是否为编号模式"""
    # 简单的正则判断：如 (1), 1), i., ii. 等
    return bool(re.match(r'^(\(\d+\)|[ivx]+\.|^\d+\))', s.strip().lower()))
...
def clean_text_thoroughly(text: str) -> str:
    ...
    # 1. Unicode 标准化 (NFKC)
    # 这会把各种复杂的兼容字符（如带圈数字、特殊组合字符）分解并转换为标准形态
    ...
    # 3. 将所有空白字符（包括 \xa0 不换行空格, \t, \n 等）统一替换为标准空格
    # \s 在 Python 正则中默认匹配各种 Unicode 空白字符
...
def extract_anchor_with_target(context: str, target_value: str, window: int = 30) -> Optional[str]:
    ...
    # 新增逻辑：如果是编号项，强制使用 strict 模式，避免 balanced 模式的正则干扰
...
if __name__ == "__main__":
    # 示例文件路径
    original_path = r"C:\Users\Administrator\Desktop\项目文件\专检\数值检查\测试文件\加粗斜体测试\原文-加粗斜体.docx"  # 请替换为原文文件路径
    translated_path = r"C:\Users\Administrator\Desktop\项目文件\专检\数值检查\测试文件\加粗斜体测试\译文-加粗斜体.docx"  # 请替换为译文文件路径
    #处理页眉
