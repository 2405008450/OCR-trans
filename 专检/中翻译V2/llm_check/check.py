...
class Match:
    ...
    def compare_texts(self, original_text, translated_text, rule):
        ...
        2) 对照原文检查译文出现错译、漏译、未翻译的情况，尤其数值错误（仅结果错误，不包括数值表达方式错误）与原文不一致的情况；
        ...
        输出要求：
        1) 输出的`原文数值`、`译文数值`、`原文上下文`、`译文上下文`、`替换锚点`字段的值必须严格按照给出的原文内容进行提取输出，不得做任何篡改；"译文修改建议值"字段必须以【原文】为唯一准则。
        ...
        输出格式示例：
         ...
         {{
            ...
            "译文数值": "第一章",
            "译文修改建议值": "Chapter I",
            "修改理由": "译文保留了中文章节序号，未按“一级：I. II. III.”规则翻译。",
            "违反的规则": "标题与编号层级：一级：I. II. III.；检查章节序号的中英映射准确性。",
            "原文上下文": "第一章  总  则",
            "译文上下文": "第一章  General Provisions",
            "替换锚点": "第一章"
         }},
        ]
        """
