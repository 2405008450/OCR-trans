...
from tools.auto_lang_tools import LANG_MAP
...
def select_language_mode():
    """让用户选择语种识别模式，返回 (mode, tool_name)
    mode: 'auto' 或 'manual'
    tool_name: 手动模式下用户选择的工具名，auto 模式下为 None
    """
    # 构建语种列表（从已注册的工具中提取，排除 claude 通用工具）
    lang_tools = [t for t in registry.tools_metadata if t["name"] != "claude"]

    print("\n" + "=" * 60)
    print("语种识别模式")
    print("=" * 60)
    print("  [0] AI 自动识别语种（默认）")
    for i, tool in enumerate(lang_tools, 1):
        print(f"  [{i}] {tool['name']} - {tool['description']}")
    print("=" * 60)

    choice = input("请选择 (直接回车=自动识别): ").strip()
    ...
    try:
        idx = int(choice)
        if 1 <= idx <= len(lang_tools):
            selected = lang_tools[idx - 1]
            print(f"[手动模式] 已选择: {selected['name']}")
            return "manual", selected["name"]
    except ValueError:
        pass

    print("[提示] 无效选择，使用 AI 自动识别")
    return "auto", None
...
def main():
    ...
    # 语种选择
    lang_mode, manual_tool_name = select_language_mode()
    
    print("\n" + "=" * 60)
    print("使用说明:")
    print("  1. 输入文件路径进行检查（支持 .xlsx, .xls, .pptx, .txt, .pdf, .docx, .xlsx, .json）")
    print("  2. 直接输入文本进行检查（只支持多行分行检查，输入空行结束）")
    print("  3. 输入 'lang' 重新选择语种模式")
    print("  4. 输入 'quit' 退出")
    current_mode = f"手动: {manual_tool_name}" if lang_mode == "manual" else "AI 自动识别"
    print(f"  当前语种模式: {current_mode}")
    print("=" * 60)
    ...
    while True:
        ...
        # 根据模式决定工具
        if lang_mode == "manual" and manual_tool_name:
            # 手动模式：直接使用用户选择的工具
        ...
        print(f"[检查文本] {arguments.get('prompt', '')[:100]}...")
