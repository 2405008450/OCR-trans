...
from replace.word.replace_revision import replace_and_revise_in_docx
...
def _build_parts(original_body, original_header, original_footer, translated_body, translated_header, translated_footer, bilingual, output_base_dir=None):
    if bilingual:
        return [
...
def _apply_word_fixes(doc, revision_manager, errors, label, region):
    if not errors:
...
def _run_word_fix_phase(translated_path, section_errors):
    ...
    body_stat = _apply_word_fixes(doc, revision_manager, section_errors.get("正文", []), "正文", "body")
    header_stat = _apply_word_fixes(doc, revision_manager, section_errors.get("页眉", []), "页眉", "header")
    footer_stat = _apply_word_fixes(doc, revision_manager, section_errors.get("页脚", []), "页脚", "footer")
...
def main():
    ...
    DEFAULT_ORIGINAL = str(
        BASE_DIR / "测试文件" / "原文-B260328127-Y-中国银行开源软件管理指引.docx")
    DEFAULT_TRANSLATED = str(
        BASE_DIR / "测试文件" / "译文-B260328127-Y-中国银行开源软件管理指引.docx")
