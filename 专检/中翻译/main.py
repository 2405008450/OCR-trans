...
from parsers.word.demo2_bock import extract_body_text
...
def _warn_empty_parse(raw: str, indent: str = "  ") -> None:
    """大模型有返回但解析为空时，打印告警和原始内容预览。"""
    if not raw or not raw.strip():
        return
    print(f"{indent}⚠️ 警告: 大模型有返回内容但解析结果为空列表，可能格式异常！")
    preview = raw.strip()[:500]
    print(f"{indent}--- 大模型原始输出 (前500字符) ---")
    print(f"{indent}{preview}")
    if len(raw.strip()) > 500:
        print(f"{indent}... (共 {len(raw.strip())} 字符，已截断)")
    print(f"{indent}-----------------------------------")
...
def _compare_with_split(matcher, orig_txt, tran_txt, rule_text, name):
    ...
    if num_parts <= 1:
        ...
        if raw:
            parsed, status = parse_json_content(raw)
            if isinstance(parsed, list):
                if status == "parse_error":
                    _warn_empty_parse(raw, indent="  ")
    ...
    for i, (orig_chunk, trans_chunk) in enumerate(pairs, 1):
        ...
        chunk_count = 0
        if chunk_result:
            parsed, status = parse_json_content(chunk_result)
            if isinstance(parsed, list):
                if status == "parse_error":
                    _warn_empty_parse(chunk_result, indent="      ")
                elif status == "no_error":
                    print(f"      ✅ 模型确认：第 {i} 块无翻译问题")
                ...
                for item in parsed:
                    ...
                    context = (item.get("译文上下文") or "").strip()
                    # 方案B：先修正重叠，再统一过滤
                    fixed = _fix_suggestion_overlap(context, tran_val, tran_sug)
                    if fixed != tran_sug:
                        print(f"      ⚙ 建议值修正: '{tran_sug}' → '{fixed}'")
                        tran_sug = fixed
                        item["译文修改建议值"] = fixed
                    if not tran_sug or tran_sug == tran_val:
                        print(f"      ⊘ 过滤: 建议值无效或与原文一致 '{tran_val}'，跳过")
                        print(f"         原始数据: {item}")
                        continue
...
def _resolve_section_output_dir(output_base_dir, section_name, leaf_name):
    base_dir = Path(output_base_dir) if output_base_dir else LLM_DIR
...
def run_comparison(
    ...
    Args:
        ...
        bilingual: 是否为中英双语对照模式（单文件，原文译文在同一文档中）
        output_base_dir: Web 任务输出根目录，不传则使用模块内默认输出目录
        ai_rule_file_path: 用户上传的规则文件路径，用于 AI 总结规则
        session_rule_text: 前端规则编辑器本次提交的规则文本，不写入磁盘
    """
    print("\n--- 阶段 1: 文本提取与 AI 对比 ---")
    if bilingual:
        print("📋 模式: 中英双语对照（单文件）")
...
def _apply_word_fixes(doc, revision_manager, errors, label, region, doc_path):
    ...
    for idx, item in enumerate(errors, 1):
        old = (item.get("译文数值") or "").strip()
        new = (item.get("译文修改建议值") or "").strip()
        reason = str(item.get("修改理由") or "数值错误").strip()
        context = item.get("译文上下文", "")
        anchor = item.get("替换锚点", "")
        ...
        if ok:
            ...
            if '<bold>' in new_raw or '<italic>' in new_raw:
                try:
                    from replace.word.format_utils import apply_format_pass
                    apply_format_pass(doc, [item])
                except Exception as fmt_e:
                    print(f"    ⚠ 格式应用异常: {fmt_e}")

            old_raw = item.get("译文数值") or ""
            if ('<bold>' in old_raw or '<italic>' in old_raw) and '<bold>' not in new_raw and '<italic>' not in new_raw:
                try:
                    from replace.word.format_utils import clear_format_pass
                    clear_format_pass(doc, [item])
                except Exception as fmt_e:
                    print(f"    ⚠ 格式清除异常: {fmt_e}")
...
def _run_word_fix_phase(translated_path, section_errors):
    ...
    try:
        ...
        print("\n📑 正在转换目录域为静态文本...")
        convert_toc_to_static(str(backup_copy_path))
    except Exception as e:
        print(f"⚠ 编号静态化异常: {e}")
...
def _run_pdf_fix_phase(translated_path, section_errors):
    ...
    with ImprovedPDFReplacer(backup_path) as replacer:
        for idx, item in enumerate(all_errors, 1):
            old_text = (item.get("译文数值") or "").strip()
            new_text = (item.get("译文修改建议值") or "").strip()
            reason = item.get("修改理由", "数值错误")
            error_type = item.get("错误类型", "数值错误")
            context = item.get("译文上下文", "")
            ...
            repl_success = result[0] if isinstance(result, tuple) and result else 0
            if repl_success > 0:
                success_count += 1
...
def run_full_pipeline(
    ...
    """
    完整流程：对比、导出报告、修复或批注。供 Web 前端任务调用。
...
def main():
    ...
    DEFAULT_ORIGINAL = str(
        BASE_DIR / "测试文件" / "关于《中国工商银行“十五五”时期发展规划》的议案0606.docx")
    DEFAULT_TRANSLATED = str(
        BASE_DIR / "测试文件" / "Proposal on the 15th Five-Year Development Plan of ICBC.docx")
  ...
  python llm/main_ddd.py -o "原文.docx" -t "译文.docx" --use-ai-rule
    ...
    if is_pdf_workflow:
        ...
        if translated_ext == ".pdf":
            ...
            try:
                ...
                # 全部为空则无需继续
                if not body_errors and not header_errors and not footer_errors:
    ...
    def apply_all_fixes(errors, label, region):
        ...
        for idx, e in enumerate(errors, 1):
            ...
            # 保留原始 new（含标签），用于格式应用；传给替换函数的是清洗后的版本（已在replace_revision内部处理）
            new_raw = e.get("译文修改建议值") or ""  # 含 <bold>/<italic> 标签的原始值
            ...
            if ok:
                ...
                if '<bold>' in new_raw or '<italic>' in new_raw:
                    try:
                        from replace.word.format_utils import apply_format_pass
                        apply_format_pass(doc, [e])
                    except Exception as fmt_e:
                        print(f"    ⚠ 格式应用异常: {fmt_e}")

                # 若 old 含格式标签而 new 不含，清除多余的加粗/斜体
                old_raw_val = e.get("译文数值") or ""
                if ('<bold>' in old_raw_val or '<italic>' in old_raw_val) and \
                   '<bold>' not in new_raw and '<italic>' not in new_raw:
                    try:
                        from replace.word.format_utils import clear_format_pass
                        clear_format_pass(doc, [e])
                    except Exception as fmt_e:
                        print(f"    ⚠ 格式清除异常: {fmt_e}")
