...
class CompletePDFExtractor:
    ...
    def _extract_full_text(self, page) -> str:
        """提取完整文本（使用 PyMuPDF 确保空格正确）"""
        try:
            # 使用 PyMuPDF 提取文本（保留空格）
            # 注意：pdfplumber 的 page 对象需要转换
            pdf_path = page.pdf.stream.name if hasattr(page.pdf.stream, 'name') else self.pdf_path

            # 打开 PyMuPDF 文档
            fitz_doc = fitz.open(pdf_path)
            fitz_page = fitz_doc[page.page_number - 1]  # pdfplumber 从 1 开始，fitz 从 0 开始
