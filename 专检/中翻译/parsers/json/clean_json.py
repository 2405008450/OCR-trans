...
def extract_and_parse(file_path: str) -> List[dict]:
    ...
    result, _ = parse_json_content(raw_content)
    return result


# 模型返回"无问题"的典型文本特征（无 JSON 对象时才生效）
_NO_ERROR_HINTS = [
    "no error", "no issue", "no problem",
    "没有发现", "未发现", "无错误", "没有错误", "无问题", "没有问题",
    "translation is correct", "翻译正确", "翻译无误",
]


def _looks_like_no_error_response(raw: str) -> bool:
    """判断模型返回是否语义上表示"没有问题"（且不含 JSON 对象）。"""
    if '{' in raw:
        return False
    lower = raw.strip().lower()
    # 空列表字面量也算正常的"无问题"响应
    if re.sub(r'\s', '', lower) in ('[]', ''):
        return True
    return any(hint in lower for hint in _NO_ERROR_HINTS)
...
def parse_json_content(raw_content: str):
    ...
    Returns:
        (list, status) 其中 status 为:
            "ok"           — 正常解析（列表为空表示模型认为无问题）
            "no_error"     — 模型明确表示无错误（自然语言或空列表）
            "parse_error"  — 有内容但无法提取到任何 JSON 对象（模型输出异常）
            "empty_input"  — 输入为空
    """
    ...
    try:
        ...
        if isinstance(data, list):
            ...
            status = "no_error" if len(data) == 0 else "ok"
            return data, status
    ...
    # 有内容但提取不到任何对象——区分"模型说没问题"和"格式异常"
    if _looks_like_no_error_response(content):
        print("✅ 模型明确表示：该段落无翻译问题")
        return [], "no_error"

    print("⚠️ 模型有返回内容但无法提取到任何 JSON 对象，可能格式异常！")
    return [], "parse_error"
