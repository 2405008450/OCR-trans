...
def _get_xml_text(element) -> str:
    """
    终极增强版：支持数学符号、Wingdings 字体符号、以及 SDT 复选框控件
    """
    # 1. 扩充映射表：包含 w:sym 引用和文本中的私有字符编码
    SYMBOL_CHAR_MAP = {
        # 数学符号
        'F0B4': '×', 'F0B8': '÷', 'F0B1': '±', 'F0B3': '≥',
        'F0A3': '≤', 'F0B9': '≠', 'F0BB': '≈',
        # Wingdings / 私有区字符 (针对 node.text 中出现的乱码)
        '\uf052': '☑', '\uf0a3': '☑', '\uf0fe': '☑', '\uf072': '☑',
        '\uf0a1': '☐', '\uf071': '☐', '\u25a1': '☐', '\u25fb': '☐',
        'F052': '☑', 'F0A1': '☐' # 针对 w:sym char 属性
    }
    ...
    for node in element.iter():
        ...
        if node.tag in (f"{{{W_NS}}}t", f"{{{M_NS}}}t"):
            if node.text:
                ...
                for raw, char in SYMBOL_CHAR_MAP.items():
                    if len(raw) > 2: # 只替换 Unicode 或预定义的字符串
                        t = t.replace(raw, char)
                texts.append(t)
            if node.tail:
                texts.append(node.tail)

        # --- B. 处理 Symbol 符号标签 (w:sym) ---
        ...
        # --- E. 图片描述 ---
            ...
            if alt: texts.append(f"[图片描述: {alt}]")
...
if __name__ == "__main__":
    ...
    # 示例文件路径
    original_path = r"C:\Users\H\Desktop\测试项目\中翻译\测试文件\译文-含不可编辑_01 (2026-007)2025年年度报告(1).docx"
    translated_path = r"C:\Users\H\Desktop\测试项目\中翻译\测试文件\译文-含不可编辑_01 (2026-007)2025年年度报告(1).docx"# trans_docx_path = translated_path
