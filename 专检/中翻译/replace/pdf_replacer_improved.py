...
from zhongfanyi.llm.llm_project.replace.pdf_annotator import PDFAnnotator
...
class ImprovedPDFReplacer(PDFAnnotator):
    ...
    def _replace_at_position(
        ...
        try:
                        ...
                        for line in block.get("lines", []):
                            for span in line.get("spans", []):
                                if "size" in span:
                                    font_size = span["size"]
                                if "font" in span:
                                    font_name = span["font"]  # ← 提取字体名称
                                if "flags" in span:
                                    font_flags = span["flags"]  # ← 提取字体标志
                                if "color" in span:
            ...
            if debug:
                print(f"    [格式] 字体: {font_name}, 大小: {font_size:.1f}, 颜色: {font_color}, 标志: {font_flags}")
            ...
            try:
                page.insert_text(
                    ...
                    fontsize=font_size,
                    color=font_color,
                    fontname=font_name  # 使用原字体
                )
                if debug:
                    print(f"    [成功] 使用原字体 '{font_name}'")
                return True
            except Exception as e:
                if debug:
                    print(f"    [警告] 原字体 '{font_name}' 不可用: {e}")
            
            # 如果原字体不可用，尝试备用字体
            supported_fonts = ["helv", "cour", "times", "symbol"]
