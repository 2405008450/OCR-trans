...
class PPTXReplacer:
    ...
    def replace_and_annotate(
        ...
        highlight=True 时对替换后的文本 run 设置红色高亮背景。
        """
        replaced = False
        ...
        if new_full == full:
            ...
            if highlight and runs:
                self._highlight_new_text_in_runs(runs, new_text)
        else:
    ...
    @staticmethod
    def _highlight_new_text_in_runs(runs, new_text: str):
        """
        精确高亮：把包含 new_text 的 run 拆成三段，只对新文本部分设红色。
        _redistribute_text 后文本全在 runs[0]，直接在那里拆分。
        """
        try:
            ...
            if not new_text or not runs:
                return

            # 找到包含 new_text 的第一个 run
            target_run = None
            for r in runs:
            ...
            full = target_run.text
            idx = full.find(new_text)
            prefix = full[:idx]
            suffix = full[idx + len(new_text):]

            para = target_run._r.getparent()
            run_idx = list(para).index(target_run._r)

            # 移除原 run
            para.remove(target_run._r)
            insert_at = run_idx

            # 前缀 run（保持原格式）
            if prefix:
                pre_r = deepcopy(target_run._r)
                pre_r.find('.//{http://schemas.openxmlformats.org/drawingml/2006/main}t').text = prefix
                para.insert(insert_at, pre_r)
                insert_at += 1

            # 新文本 run（红色）
            new_r = deepcopy(target_run._r)
            t_elem = new_r.find('.//{http://schemas.openxmlformats.org/drawingml/2006/main}t')
            if t_elem is not None:
                t_elem.text = new_text
            rPr = new_r.find('{http://schemas.openxmlformats.org/drawingml/2006/main}rPr')
            if rPr is None:
                rPr = etree.SubElement(new_r, '{http://schemas.openxmlformats.org/drawingml/2006/main}rPr')
                new_r.insert(0, rPr)
            # 移除已有颜色，设为红色
            ...
            para.insert(insert_at, new_r)
            insert_at += 1

            # 后缀 run（保持原格式）
            if suffix:
                suf_r = deepcopy(target_run._r)
                suf_r.find('.//{http://schemas.openxmlformats.org/drawingml/2006/main}t').text = suffix
                para.insert(insert_at, suf_r)
    ...
    def _add_comment(self, slide, old_text: str, new_text: str, reason: str):
        ...
        if reason:
            comment_text += f"\n理由: {reason}"
        try:
            self._add_red_textbox(slide, comment_text)
            self.annotations_count += 1
        except Exception as e:
            print(f"  [警告] 文本框批注失败({e})，改用备注记录")
            self._append_note_fallback(slide, old_text, new_text, reason)
            self.annotations_count += 1

    def _add_red_textbox(self, slide, text: str):
        """在幻灯片右上角添加红色小文本框作为修改标注。"""
        from pptx.util import Inches, Pt
        from pptx.dml.color import RGBColor
        from pptx.enum.text import PP_ALIGN

        existing_boxes = [s for s in slide.shapes if s.name.startswith("_修改标注_")]
        left = slide.slide_layout.slide_master.slide_width - Inches(3.2)
        top = Inches(0.05) + len(existing_boxes) * Inches(0.65)

        txBox = slide.shapes.add_textbox(left, top, Inches(3.1), Inches(0.6))
        txBox.name = f"_修改标注_{len(existing_boxes)}"
        tf = txBox.text_frame
        tf.word_wrap = True
