...
def _execute_replace(paragraph, old_value: str, new_value: str, reason: str,
    ...
    # 目录行匹配 — old_value 末尾是数字，中间可能有省略号/Tab
    # 用 toc 模式在原始文本中定位整行，然后整行替换为 new_value
    if re.search(r'\d+$', old_clean):
        toc_pat = build_smart_pattern(old_clean, mode="toc")
        if toc_pat:
            m = re.search(toc_pat, full_text, re.IGNORECASE | re.DOTALL)
            if m:
                actual_old = m.group(0)
                return revision_manager.replace_in_paragraph(paragraph, actual_old, new_value, reason=reason)
...
def replace_and_revise_in_docx(
    ...
    from replace.word.format_utils import strip_format_tags
    old_value_raw = (old_value or "").strip()   # 保留原始标签，供跨段落处理
    new_value_raw = (new_value or "").strip()
    old_value = strip_format_tags(old_value_raw)
    new_value = strip_format_tags(clean_text_thoroughly(new_value_raw).strip())
    ...
    # ── 内部辅助：替换成功后统一处理格式标签 ──────────────────────
    def _handle_format_after_replace(strategy: str) -> Tuple[bool, str]:
        """替换成功后，根据 old/new 的格式标签差异自动应用/清除加粗斜体。"""
        try:
            from replace.word.format_utils import apply_format_pass, clear_format_pass
            err_item = {
                "译文数值": old_value_raw,
                "译文修改建议值": new_value_raw,
                "译文上下文": context,
            }
            old_has_fmt = ("<bold>" in old_value_raw or "<italic>" in old_value_raw)
            new_has_fmt = ("<bold>" in new_value_raw or "<italic>" in new_value_raw)

            if new_has_fmt:
                # new 含格式标签 → 应用加粗/斜体
                apply_format_pass(doc, [err_item])
            elif old_has_fmt and not new_has_fmt:
                # old 含格式标签、new 不含 → 清除多余格式
                clear_format_pass(doc, [err_item])
        except Exception as _fmt_e:
            print(f"    ⚠ 格式处理异常: {_fmt_e}")
        return True, strategy

    # ===== 跨段落路由：old_value 含 \n 时走多段落替换逻辑 =====
    if "\n" in old_value:
        from replace.word.replace_multiparagraph import replace_multiparagraph
        return replace_multiparagraph(
            doc, old_value_raw, new_value_raw, reason,
            revision_manager, context=context, region=region
        )
    ...
    if is_list_pattern(old_value) and not getattr(doc, '_numbering_staticized', False):
        ...
        if is_same_format:
            ...
            if ok:
                return _handle_format_after_replace(strategy)
            # 失败则继续走后续策略
        else:
            try:
                ...
                if success:
                    return _handle_format_after_replace(f"Word自动编号替换: {message}")
                else:
    ...
    if anchor_text:
        ...
        if anchor_clean:
            ...
            for p in paragraph_iterator():
                ...
                full_clean = clean_text_thoroughly(full_text)
                full_norm = _normalize_numbering_spaces(full_clean)
                # 尝试原始清洗匹配 + 编号归一化匹配 + 中文后缀剥离匹配 + 目录行匹配
                toc_hit = False
                if not (anchor_clean in full_clean
                        or anchor_norm in full_norm
                        or (anchor_stripped and anchor_stripped != anchor_clean
                            and anchor_stripped in full_clean)):
                    # 目录行 fallback：锚点末尾是数字时，用 toc 模式正则匹配
                    if re.search(r'\d+$', anchor_clean):
                        toc_pat = build_smart_pattern(anchor_clean, mode="toc")
                        if toc_pat and re.search(toc_pat, full_clean, re.IGNORECASE | re.DOTALL):
                            toc_hit = True
            ...
            if anchor_candidates:
                ...
                for _, _, cand_para, cand_text in anchor_candidates:
                    ...
                    if ok:
                        return _handle_format_after_replace(f"锚点匹配 [{region_desc}]")
    ...
    if context:
        ...
        if context_anchor:
            ...
            for _, cand_para in ctx_candidates:
                ok = _execute_replace(cand_para, old_value, new_value, reason, revision_manager)
                if ok:
                    return _handle_format_after_replace(f"上下文锚点匹配 [{region_desc}]")
    ...
    for strategy_name, pattern, min_similarity, max_level in strategies:
        ...
        ok = _execute_replace(best_para, old_value, new_value, reason, revision_manager)
        if ok:
            return _handle_format_after_replace(f"{strategy_name} [{region_desc}] (得分:{best_score:.2f}, 候选:{len(candidates)})")

    # ===== 策略7：脚注/尾注（正文未找到时才检查，避免走错路径） =====
    if region in ("body", "all"):
        try:
            from replace.word.footnote_replacer import check_text_in_footnotes
...
def flush_footnote_replacements(doc, save_path: str) -> int:
    ...
    from replace.word.footnote_replacer import replace_in_footnotes_xml
