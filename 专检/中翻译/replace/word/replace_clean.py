...
from note.pizhu import CommentManager
...
def build_smart_pattern(s: str, mode: str = "balanced") -> str:
    ...
    if mode == "toc":
        # 目录行模式：把字符串拆成"标题部分"和"末尾页码"两段
        # 末尾页码 = 字符串末尾连续的数字
        # 中间允许任意字符（省略号、Tab、空格等），但限制长度避免跨段误匹配
        m = re.match(r"^(.*?)(\d+)$", s, flags=re.DOTALL)
        if m:
            title_part = m.group(1).rstrip()
            page_num = m.group(2)
            title_pattern = build_smart_pattern(title_part, mode="balanced")
            return title_pattern + r"[\s\S]{0,60}?" + re.escape(page_num)
        # 没有末尾数字则退化为 balanced
        return build_smart_pattern(s, mode="balanced")
...
def iter_body_paragraphs(doc: Document):
    """
    仅遍历正文段落（包括表格、文本框、脚注、尾注，但不包括页眉页脚）
        ...
        if hasattr(doc, '_part') and hasattr(doc._part, 'package'):
            ...
            for part in package.iter_parts():
                if part.partname.endswith('footnotes.xml'):
                    ...
                    # 遍历所有脚注
                    for footnote_elem in root.findall(f".//{{{W_NS}}}footnote"):
                        # 遍历脚注中的段落
                        for para_elem in footnote_elem.findall(f".//{{{W_NS}}}p"):
                            try:
                                from docx.text.paragraph import Paragraph
                                # 尝试创建段落对象
                                p = Paragraph(para_elem, doc)
                                # 测试是否能访问 runs（验证对象是否有效）
                                _ = p.runs
                                yield p
    ...
    # 遍历尾注 (Endnotes) - 使用 XML 直接访问
    try:
        if hasattr(doc, '_part') and hasattr(doc._part, 'package'):
            package = doc._part.package
            
            for part in package.iter_parts():
                if part.partname.endswith('endnotes.xml'):
                    from lxml import etree
                    endnotes_xml = part.blob
                    root = etree.fromstring(endnotes_xml)
                    
                    for endnote_elem in root.findall(f".//{{{W_NS}}}endnote"):
                        for para_elem in endnote_elem.findall(f".//{{{W_NS}}}p"):
                            try:
                                from docx.text.paragraph import Paragraph
                                p = Paragraph(para_elem, doc)
                                _ = p.runs
