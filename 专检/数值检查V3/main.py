...
from llm.llm_project.parsers.excel.excel_parser import parse_excel_with_pandas
from llm.llm_project.parsers.excel.excel装载 import ExcelReportGenerator
from llm.llm_project.parsers.pptx.pptx_parser import parse_pptx
from llm.llm_project.replace.excel.excel_replacer import ExcelReplacer
from llm.llm_project.replace.pptx.pptx_replacer import PPTXReplacer
from llm.llm_project.divide.text_splitter import split_text_pair, _count_chars
from llm.llm_project.llm_check.check_2 import run_bilingual_comparison

PROJECT_ROOT = Path(__file__).resolve().parent
LLM_PROJECT_ROOT = PROJECT_ROOT / "llm" / "llm_project"
SECTION_DIRS = {
    "正文": "zhengwen",
    "页眉": "yemei",
    "页脚": "yejiao",
...
def get_output_dir(section_name: str, output_type: str) -> str:
    """返回当前 V3 项目下的输出目录。"""
    return str(LLM_PROJECT_ROOT / SECTION_DIRS[section_name] / f"output_{output_type}")


def get_default_input_paths():
    """优先读取环境变量，否则使用当前项目目录下的占位文件名。"""
    return (
        os.environ.get("NUMBER_CHECK_ORIGINAL", str(PROJECT_ROOT / "原文.docx")),
        os.environ.get("NUMBER_CHECK_TRANSLATED", str(PROJECT_ROOT / "译文.docx")),
    )


def generate_excel_reports(generator, body_errors, header_errors, footer_errors):
    """按区域输出 Excel 报告，统一使用当前 V3 目录。"""
    results = {}
    for section_name, errors in {
        ...
        file_path, data = generator.get_excel(errors, output_dir=get_output_dir(section_name, "excel"))
        print(f"✓ {section_name}报告已存至: {file_path}")
        results[section_name] = (file_path, data)
    return results
...
def _compare_with_split(matcher, orig_txt, tran_txt, name):
    ...
    """
    from llm.llm_project.parsers.json.clean_json import parse_json_content
...
def main():
    # 1) 配置默认路径
    DEFAULT_ORIGINAL, DEFAULT_TRANSLATED = get_default_input_paths()
    ...
    parser.add_argument("--bilingual", action="store_true",
                        help="启用单文件双语对照模式，此时仅使用 --original 指定待检文件")
    ...
    if args.bilingual:
        if not os.path.exists(args.original):
            print(f"❌ 错误: 输入的文件路径不存在: {args.original}")
            return
        print("📋 模式: 单文件双语对照检查")
        report_paths = run_bilingual_comparison(
            args.original,
            output_json_dir=get_output_dir("正文", "json")
        )
    ...
    if is_pdf_workflow:
        ...
        if translated_ext == ".pdf":
            ...
            try:
                ...
                generate_excel_reports(e, body_errors, header_errors, footer_errors)
    ...
    if is_excel_workflow:
        ...
        try:
            ...
            generate_excel_reports(eg, body_errors, header_errors, footer_errors)
    ...
    def apply_all_fixes(errors, label, region):
        ...
        for idx, e in enumerate(errors, 1):
            ...
            # 检查是否为编号模式，且已经被批量处理过
            from llm.llm_project.replace.replace_clean import is_list_pattern
