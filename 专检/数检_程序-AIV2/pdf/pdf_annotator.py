...
from pdf.text_matcher import TextMatcher, generate_search_variants
    ...
    def _verify_highlighted_text(self, instance: Dict, search_text: str, debug: bool = False) -> bool:
        ...
        try:
            ...
            if len(normalized_search) <= 3:
                ...
                # 对于长文本，使用包含检查
                # 优先用 instance 里记录的实际搜索命中文本（search_for 已验证）
                instance_text = instance.get('text', '')
                if instance_text:
                    norm_instance = ''.join(instance_text.split())
                    # search_for 命中的文本 == search_text（或其变体），直接认为有效
                    match = (normalized_search in norm_instance or
                             norm_instance in normalized_search or
                             # 去非字母数字后比较（应对 Symbol 乱码）
                             re.sub(r'[^a-z0-9\u4e00-\u9fff/]', '', normalized_search.lower()) in
                             re.sub(r'[^a-z0-9\u4e00-\u9fff/]', '', norm_instance.lower()))
                else:
                    # 兜底：用 rect 提取的文本，清洗掉 Symbol 乱码后比较
                    def _alnum(t: str) -> str:
                        return re.sub(r'[^a-z0-9\u4e00-\u9fff/]', '', t.lower())
                    match = (_alnum(normalized_search) in _alnum(normalized_highlight) or
                             _alnum(normalized_highlight) in _alnum(normalized_search) or
                             _alnum(normalized_search) in _alnum(normalized_textbox) or
                             _alnum(normalized_textbox) in _alnum(normalized_search))
