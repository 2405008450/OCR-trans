...
def _find_sdt_wrapped_content(container_elem, w_ns):
    """
    找出 container_elem（如 w:body / w:hdr / w:ftr / w:tc）直接子元素中，
    被 w:sdt（内容控件）包裹的顶层 w:p / w:tbl 元素，按相对顺序返回。

    背景：python-docx 的 paragraphs/tables 属性（底层 p_lst/tbl_lst）只匹配
    容器的直接 w:p/w:tbl 子元素，完全跳过 w:sdt 包裹的内容。Word 生成的
    目录（TOC）等常把每个条目段落包在 w:sdt 里，导致这些段落在替换定位
    时彻底不可见——不仅不在候选集合里，甚至会让"唯一候选"误判为其它
    重复文本所在的段落（因为真正该改的那处压根没被扫描到）。

    支持嵌套 w:sdt（sdtContent 内再套 sdt）。
    """
    W_sdt = f"{{{w_ns}}}sdt"
...
def _iter_sdt_paragraphs(container_elem, parent, w_ns):
    """
    对 container_elem 直接子元素中被 w:sdt 包裹的段落/表格递归展开为 Paragraph 对象。
    与 _find_sdt_wrapped_content 配套使用，供 iter_body/header/footer_paragraphs 调用。
    """
    ...
    for elem in _find_sdt_wrapped_content(container_elem, w_ns):
        if elem.tag == W_p:
            yield Paragraph(elem, parent)
        elif elem.tag == W_tbl:
            tbl = Table(elem, parent)
            for row in tbl.rows:
                for cell in row.cells:
                    yield from cell.paragraphs
                    # 单元格内也可能有被 sdt 包裹的段落
                    yield from _iter_sdt_paragraphs(cell._element, cell, w_ns)
...
def iter_all_paragraphs(doc: Document, include_headers_footers: bool = True):
    ...
    # 被 w:sdt 内容控件包裹的正文段落（如目录 TOC），doc.paragraphs 不会包含
    _w_ns = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
    for p in _iter_sdt_paragraphs(doc.element.body, doc, _w_ns):
        yield p
    ...
    if include_headers_footers:
        for sec in doc.sections:
            ...
            for p in sec.footer.paragraphs:
                yield p
            for p in _iter_sdt_paragraphs(sec.header._element, sec.header, _w_ns):
                yield p
            for p in _iter_sdt_paragraphs(sec.footer._element, sec.footer, _w_ns):
                yield p
...
def iter_body_paragraphs(doc: Document):
    ...
    # 被 w:sdt 内容控件包裹的段落/表格（如 Word 自动生成的目录 TOC 常见此结构）
    # doc.paragraphs / doc.tables 完全跳过这些内容，必须单独展开
    for p in _iter_sdt_paragraphs(doc.element.body, doc, W_NS):
        yield from yield_paragraph(p._element, p._parent)
...
def iter_header_paragraphs(doc: Document):
    ...
    for sec in doc.sections:
        ...
        # 被 w:sdt 内容控件包裹的段落/表格
        for p in _iter_sdt_paragraphs(sec.header._element, sec.header, W_NS):
            yield from yield_paragraph(p._element, p._parent)
...
def iter_footer_paragraphs(doc: Document):
    ...
    for sec in doc.sections:
        ...
        # 被 w:sdt 内容控件包裹的段落/表格
        for p in _iter_sdt_paragraphs(sec.footer._element, sec.footer, W_NS):
            yield from yield_paragraph(p._element, p._parent)
