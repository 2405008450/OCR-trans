...
class Match:
    ...
    def compare_texts(self, original_text):
        ...
        3. **数值零误差：** 严格检查单位数值翻译后是否一致（如4亿和40 million是否一致）,严禁四舍五入;但是要注意文本数字兼容：原文“十四”，译文“14”或“14th”应根据语境判定。如果语境是第14条，则一致；除非译文拼写错误或数值变动（如 14 变成 24）才标记为错误。       
        4. **编号连续性：** 检查 Article, Section, (i), (1) 等层级编号是否跳号、重号或者译文连续编号混用不同符号如1. II.3. ...。
        ...
        - 原文：10.00 | 译文：10 -> [一致，无需修改]
        - 原文：1007.3 | 译文：1,007.30 -> [一致，无需修改]
        - 原文：1.27亿 | 译文：127 million -> [一致，无需修改]
        - 原文：10 trillion | 译文：10,000 billion -> [一致，无需修改]
        - 原文：三 | 译文：3/three -> [一致，无需修改]    
        - 原文：one | 译文：1 -> [一致，无需修改]
        ...
        - 原文：1234.16亿 | 译文：123.42 billion -> [数值错误：精度丢失,严禁四舍五入]
        ...
        仅输出 JSON 数组，不得包含说明文字。若无错误，则输出 `[]`。格式如下：
            [
              ...
              {{
                ...
                "译文上下文": "Article 6 Data demand management shall be in compliance with the following principles:",
                "原文数值": "第二条",
                "译文数值": "Article 6",
                "替换锚点": "Article 6",
                "译文修改建议值": "Article 2",
                "错误类型": "标题与编号层级",
                "修改理由": "译文阿拉伯数字错误（原文为二，译文误写为6）。",
...
def _compare_bilingual_with_split(matcher, text, name):
    ...
    分块规则（由 split_bilingual_text 实现）：
    - 遍历段落，遇到中文段开始一组，后面的英文段和表格行都归入这一组
    - 只在英文段后面（即一组结束时）才允许切分
    - 表格行（含 \\t 或多个 |）无条件跟随当前组，不会成为切分点
    - 累积到接近 10000 字时在下一个组边界切
    - 块之间重叠 2 个段落对
    ...
    Returns:
        list[dict] — 解析后的错误列表（可能为空）
    """
    from parsers.json.clean_json import parse_json_content
    from divide.text_splitter import split_text, auto_num_parts, _count_chars
...
def run_bilingual_comparison(file_path, output_json_dir=None):
    """单文件双语对照检查入口。

    提取文档正文/页眉/页脚，对每个区域使用双语分块策略进行 LLM 检查，
    生成 JSON 报告。

    Args:
        file_path: 双语对照文档路径（支持 docx/doc/pdf/xlsx/pptx）
        output_json_dir: JSON 输出目录（默认自动生成）

    Returns:
        dict  各区域的 JSON 报告路径 {"正文": path, "页眉": path, "页脚": path}
    """
    ...
    if output_json_dir is None:
        output_json_dir = str(Path(__file__).resolve().parents[1] / "zhengwen" / "output_json")

    matcher = Match()

    parts = [
        ("正文", body, output_json_dir),
        ("页眉", header,
         str(Path(__file__).resolve().parents[1] / "yemei" / "output_json")),
        ("页脚", footer,
         str(Path(__file__).resolve().parents[1] / "yejiao" / "output_json")),
    ]
    ...
    for name, txt, out_dir in parts:
        ...
        _, json_path = write_json_with_timestamp(res, out_dir)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(args.file):
        ...
        run_bilingual_comparison(args.file, output_json_dir=args.output)
