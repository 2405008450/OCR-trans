    ...
    if num_parts <= 1:
        ...
        if raw:
            ...
            if isinstance(parsed, list):
                ...
                filtered = []
                for item in parsed:
                    tran_val = strip_format_tags((item.get("译文数值") or "").strip())
                    tran_sug = strip_format_tags((item.get("译文修改建议值") or "").strip())
...
def run_comparison(original_path, translated_path, use_ai_rule=False):
    ...
    parts = [
        ("正文", original_body, translated_body,
         str(LLM_DIR / "zhengwen" / "output_json")),
        ("页眉", original_header, translated_header,
         str(LLM_DIR / "yemei" / "output_json")),
        ("页脚", original_footer, translated_footer,
         str(LLM_DIR / "yejiao" / "output_json"))
    ]
    report_paths = {}
...
def main():
    ...
    # 1) 配置默认路径
    DEFAULT_ORIGINAL = r"C:\Users\H\Desktop\测试项目\数检\王露数字强化练习_Day1.docx"
    DEFAULT_TRANSLATED = r"C:\Users\H\Desktop\测试项目\数检\王露数字强化练习_Day1.docx"
    ...
    if BILINGUAL_MODE:
        ...
        if suffix in [".docx", ".doc"]:
            ...
            # 自动编号静态化
            try:
                from replace.numbering_to_static import convert_numbering_to_static, has_auto_numbering
                if has_auto_numbering(str(backup_copy_path)):
                    print("\n🔢 检测到自动编号，正在转换为静态文本...")
                    if convert_numbering_to_static(str(backup_copy_path)):
                        print("✓ 自动编号已转为静态文本")
                    else:
                        print("⚠ 自动编号静态化失败")
            except Exception as e:
            ...
            def apply_all_fixes(errors, label, region):
                ...
                for idx, e in enumerate(errors, 1):
                    ...
                    if ok:
                        ...
                        # 替换成功后立即应用加粗/斜体格式
                        new_raw = e.get("译文修改建议值") or ""
                        if '<bold>' in new_raw or '<italic>' in new_raw:
                            try:
                                from replace.format_utils import apply_format_pass
    ...
    if is_pdf_workflow:
        ...
        if translated_ext == ".pdf":
            print("\n--- 阶段 2: PDF 批注 ---")
            try:
                from replace.pdf_annotator import PDFAnnotator
                from backup_copy.backup_manager import ensure_backup_copy
                ...
                print("\n--- 生成Excel报告 ---")
                e = ExcelReportGenerator()
                body_file_path, _ = e.get_excel(body_errors,
                                                output_dir=str(Path(__file__).parent / "zhengwen" / "output_excel"))
                print(f"✓ 正文报告已存至: {body_file_path}")
                header_file_path, _ = e.get_excel(header_errors,
                                                  output_dir=str(Path(__file__).parent / "yemei" / "output_excel"))
                print(f"✓ 页眉报告已存至: {header_file_path}")
                footer_file_path, _ = e.get_excel(footer_errors,
                                                  output_dir=str(Path(__file__).parent / "yejiao" / "output_excel"))
                print(f"✓ 页脚报告已存至: {footer_file_path}")
    ...
    if is_excel_workflow:
        print("\n--- 阶段 2: Excel 替换与批注 ---")
        try:
            from backup_copy.backup_manager import ensure_backup_copy as ensure_backup
