        ...
        for axis_tag, axis_name in axis_tags:
            for axis in root.iter(axis_tag):
                ...
                if title_elem is not None:
                    parts = [at.text.strip() for at in title_elem.iter(f"{{{A_NS}}}t")
                             if at.text]
                    if parts:
            ...
            for child in ext.iter():
                ...
                if local == "datalabelsRange":
                    for cache_child in child.iter():
                        cache_local = (etree.QName(cache_child.tag).localname
                                       if isinstance(cache_child.tag, str) else "")
...
# ============================================================
#  文本提取（增加 skip_containers 参数）
# ============================================================
def _get_xml_text(element, *, skip_containers: bool = True) -> str:
    """
...
def _process_anchored_content(p_element, loader: DocAnchorsLoader) -> List[str]:
    ...
    for txbx in p_element.iter(f"{{{WPS_NS}}}txbxContent"):
        ...
        text = _get_xml_text(txbx, skip_containers=False)
        if text:
            extras.append(text)
    ...
    for v_txbx in p_element.iter(f"{{{V_NS}}}textbox"):
        ...
        text = _get_xml_text(v_txbx, skip_containers=False)
        if text:
            extras.append(text)
...
def extract_body_text(doc_path: str) -> str:
    ...
    def _process_paragraph(p_elem) -> List[str]:
        ...
        # 正文提取时跳过文本框和图表内部节点
        text = _get_xml_text(p_elem, skip_containers=True)
...
if __name__ == "__main__":
    ...
    # 示例文件路径
    original_path = r"C:\Users\H\Desktop\测试项目\中翻译\测试文件\雅本化学2025ESG报告文字稿-20260409.docx"
    translated_path = r"C:\Users\H\Desktop\测试项目\中翻译\测试文件\译文-B260328127-Y-中国银行开源软件管理指引.docx"
