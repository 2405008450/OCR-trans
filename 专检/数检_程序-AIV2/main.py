...
def _build_pairs_from_docx(src_file: str, tgt_file: str) -> list:
    ...
    print("📖 提取原文...")
    src_segs = scan_docx(src_file)
    print("📖 提取译文...")
    tgt_segs = scan_docx(tgt_file)

    # 过滤掉页眉/页脚片段，只保留正文相关内容
    # 页眉/页脚走独立检查通道（header_extractor / footer_extractor），
    # 混入正文 pairs 会导致页眉错误用 region="body" 误替换正文
    _BODY_SOURCES = {"body", "toc", "table", "footnote", "endnote", "chart", "textbox"}
    ...
    return [(s.text, t.text, t.para_index, t.row_context, t.source) for s, t in zip(src_segs, tgt_segs)]
    ...
    if fmt in ("pdf", "pptx"):
        ...
        return [
            (src_segs[i].text if i < len(src_segs) else "",
             ...
             tgt_segs[i].source if i < len(tgt_segs) else "body")
...
def apply_revisions_from_ai_map(
        ...
        region: str = "body",  # 默认区域（当 row 无 tgt_source 时使用）
        doc_path: str = None,
) -> list:
    """
    ...
    写回区域优先级：row["tgt_source"] > 参数 region（兜底默认值）
    source → region 映射：
      body/table/toc/textbox/chart → "body"（均位于正文 XML）
      header                       → "header"
      footer                       → "footer"
      footnote                     → "footnote"
      endnote                      → "endnote"
    ...
    # source 字段 → replace_and_revise_in_docx region 参数映射
    _SOURCE_TO_REGION = {
        "body":     "body",
        "table":    "body",
        "toc":      "body",
    ...
    for idx, _ in enumerate(final_rows):
        ...
        for err_no, err in enumerate(ai.get("errors", []), 1):
            ...
            # 按 tgt_source 确定写回区域，未记录时沿用调用方传入的 region 默认值
            tgt_source  = row_data.get("tgt_source", "")
            row_region  = _SOURCE_TO_REGION.get(tgt_source, region)
            ...
            tasks.append((idx, err_no, old_val, new_val, context, anchor, reason, para_index, prev_tgt, next_tgt, row_region))
    ...
    tasks_with_occ = []
    for task in tasks:
        idx, err_no, old_val, new_val, context, anchor, reason, para_index, prev_tgt, next_tgt, row_region = task
        occ = _occ_counter[old_val]
        _occ_counter[old_val] += 1
        tasks_with_occ.append((idx, err_no, old_val, new_val, context, anchor, reason, para_index, occ, prev_tgt, next_tgt, row_region))

    success, failed = 0, 0
    for i, (idx, err_no, old_val, new_val, context, anchor, reason, para_index, occ, prev_tgt, next_tgt, row_region) in enumerate(tasks_with_occ, 1):
        ok, strategy = replace_and_revise_in_docx(
            ...
            region=row_region, doc_path=doc_path, para_cache=None,
        ...
        if ok:
            success += 1
            if "批注兜底" in strategy:
                print(f"  📌 [{i:>2}/{len(tasks)}] [{row_region}] '{old_val}' → '{new_val}'  ({strategy})")
            else:
                print(f"  ✅ [{i:>2}/{len(tasks)}] [{row_region}] '{old_val}' → '{new_val}'  ({strategy})")
        else:
            failed += 1
            print(f"  ⚠️  [{i:>2}/{len(tasks)}] [{row_region}] 未找到: '{old_val}'  ({strategy})")
...
# =============================================================
# 旧版流程（Mode B · DOCX · 无程序对齐，完全对齐 old_check/old_number/main.py）
# =============================================================

def _run_legacy_mode(src_docx_path: str,
    ...
    docx_path = write_target
    os.makedirs(output_dir, exist_ok=True)
    stem = Path(src_docx_path).stem if bilingual_mode else Path(tgt_docx_path).stem

    # ── 阶段1：提取 + AI 对比（无规则预检） ──────────────────────────
    errors_by_region = run_legacy_comparison(
        src_docx_path, tgt_docx_path, bilingual_mode=bilingual_mode)

    total_errors = sum(len(v) for v in errors_by_region.values())
    if total_errors == 0:
        print("\n✅ 共发现0个需要修改的对象，跳过 Excel 报告和修订文件生成。")
        return errors_by_region

    # 保存原始 JSON 供调试/复用
    json_path = os.path.join(output_dir, f"{stem}_legacy_errors.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(errors_by_region, f, ensure_ascii=False, indent=2)
    print(f"  💾 旧版错误JSON: {json_path}")

    # ── 阶段2：生成 Excel 报告（错误列表，非全量行审计） ─────────────
    print("\n" + "="*60)
    print("📊 生成报告（旧版模式：错误列表）")
    print("="*60)
    report_path = os.path.join(output_dir, f"{stem}_legacy_output.xlsx")
    generate_legacy_report(errors_by_region, report_path)

    # ── 阶段3：写入修订（Track Changes，仅锚点/上下文定位） ──────────
    print("\n" + "="*60)
    print("📝 写入修订（旧版模式）")
    print("="*60)
...
def run(alignment_path: str = None,
    ...
    公共参数：
      ...
      use_legacy_mode  : True 时（仅 模式B + docx）完全采用旧版流程：
                             ...
                             报告降级为错误列表（非全量行审计）。
      bilingual_mode   : 仅在 use_legacy_mode=True 时生效。
    ...
    if use_legacy_mode:
        return _run_legacy_mode(
            ...
            tgt_docx_path=tgt_docx_path,
            docx_path=docx_path,
            output_dir=output_dir,
            revised_docx_path=revised_docx_path,
    ...
    if alignment_path:
        ...
        # 把 para_index / row_context / tgt_source 写入每个 row，供写回阶段精确定位
        for i, row in enumerate(body_rows):
            if i < len(pairs) and len(pairs[i]) >= 4:
                row["para_index"]  = pairs[i][2]
                row["row_context"] = pairs[i][3]
                row["tgt_source"]  = pairs[i][4] if len(pairs[i]) >= 5 else "body"
    ...
    def _ai_map_to_flat_errors(rows, ai_map):
        ...
        同时写入 prev_tgt/next_tgt，供 quick_replace.py 做夹逼定位。
        """
        out = []
        global_err_no = 1
        for idx, row in enumerate(rows):
            ...
            if not entry:
                continue
            prev_tgt = rows[idx - 1].get("译文", "") if idx > 0 else ""
            next_tgt = rows[idx + 1].get("译文", "") if idx + 1 < len(rows) else ""
    ...
    if docx_path:
        ...
        if fmt == "docx":
            ...
            # 修订前将所有自动编号转为静态文本，避免替换定位失败或编号错乱
            try:
                from numbering_to_static import convert_numbering_to_static, has_auto_numbering, convert_toc_to_static
                if has_auto_numbering(out_path):
                    print("🔢 检测到自动编号，转换为静态文本...")
            ...
            # 修复指向缺失部件的悬空关系（如 Target="NULL"），
            # 否则 python-docx 打开时会抛 KeyError: "There is no item named 'word/NULL'..."
            try:
                from docx_repair import repair_dangling_rels
                if repair_dangling_rels(out_path):
                    print("  🔧 已修复损坏关系")
            ...
            _, s, f = apply_revisions_from_ai_map(
                body_final, body_ai_map, doc, rm, region="body", doc_path=out_path)
            ...
            if header_final and header_ai_map:
                ...
                _, s, f = apply_revisions_from_ai_map(
                    header_final, header_ai_map, doc, rm, region="header", doc_path=out_path)
                total_success += s; total_failed += f
            ...
            if footer_final and footer_ai_map:
                ...
                _, s, f = apply_revisions_from_ai_map(
                    footer_final, footer_ai_map, doc, rm, region="footer", doc_path=out_path)
                total_success += s; total_failed += f
...
if __name__ == "__main__":
    ...
    #   use_total_normalizer=False : normalizer（基础策略）；
    #                                True 时切换为 normalizer_total（支持货币/日期/分数/季度等更多规则）

    #   use_legacy_mode = True,  # 仅传原文+译文模式必须启用use_legacy_mode参数

    # ── 用法：传原文+译文+对照文件，对齐检查────────────
    # run(
    #     src_docx_path=r"D:\project\数检_程序-AI\测试文件\雅本化学2025ESG报告文字稿-20260409.docx",
    #     tgt_docx_path=r"D:\project\数检_程序-AI\新致力公司产品手册设计（翻译版本）_translated.docx",
    #     alignment_path=r"D:\project\数检_程序-AI\新致力公司产品手册设计（翻译版本）-bilingual.xlsx",
    #     output_dir=r"D:\project\数检_程序-AI\output",
    #     revision_author="数值检查",
    #     #ai_check_all=True,
    #     use_total_normalizer=True,
    # )

    # ── 用法（旧版流程·双文件模式）：分别传原文+译文两个文件，跳过程序对齐，完全走旧版直送AI逻辑────
    #   use_legacy_mode=True 必填；bilingual_mode=False（默认）表示双文件模式
    run(
        src_docx_path=r"D:\project\数检_程序-AI\新致力公司产品手册设计（翻译版本）.docx",
        tgt_docx_path=r"D:\project\数检_程序-AI\新致力公司产品手册设计（翻译版本）_translated.docx",
        output_dir=r"D:\project\数检_程序-AI\output",
        revision_author="数值检查",
        use_legacy_mode=True,  # 此模式必须启用 use_legacy_mode 参数
        # bilingual_mode=False,  # 双文件模式（默认），传两个独立文件
    )

    # ── 用法（旧版流程·单文件双语对照模式）：只上传一个文件（文档内中英段落交替）────
    #   use_legacy_mode=True 必填；bilingual_mode=True 表示单文件模式，无需 tgt_docx_path
    # run(
    #     src_docx_path=r"D:\project\数检_程序-AI\old_check\old_number\王露数字强化练习_Day1.docx",
    #     output_dir=r"D:\project\数检_程序-AI\output",
    #     revision_author="数值检查",
