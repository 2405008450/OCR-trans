...
class jsonParser:
    ...
    def load_error_json_text(raw: str) -> List[Dict[str, Any]]:
        ...
        key_map = [
            ...
            ("错误类型", r"错误\s*类型\s*[:：]\s*(.+?)(?=译文\s*数值\s*[:：]|$)"),
            ("原文数值", r"译文\s*数值\s*[:：]\s*(.+?)(?=译文\s*数值\s*[:：]|$)"),
            ("译文数值", r"译文\s*数值\s*[:：]\s*(.+?)(?=译文\s*修改\s*建议\s*值\s*[:：]|修改\s*理由\s*[:：]|$)"),
            ("译文修改建议值", r"译文\s*修改\s*建议\s*值\s*[:：]\s*(.+?)(?=修改\s*理由\s*[:：]|$)"),
            ("修改理由", r"修改\s*理由\s*[:：]\s*(.+?)(?=违反\s*的\s*规则\s*[:：]|$)"),
            ("违反的规则", r"违反\s*的\s*规则\s*[:：]\s*(.+?)(?=译文\s*上下文\s*[:：]|$)"),
            ("原文上下文", r"译文\s*上下文\s*[:：]\s*(.+?)(?=译文\s*上下文\s*[:：]|$)"),
            ("译文上下文", r"译文\s*上下文\s*[:：]\s*(.+?)(?=译文\s*位置\s*[:：]|$)"),
            ("原文位置", r"译文\s*位置\s*[:：]\s*(.+?)(?=译文\s*位置\s*[:：]|$)"),
...
if __name__ == '__main__':
    body_result_path = r"C:\Users\Administrator\Desktop\中翻译规则检查\llm\llm_project\zhengwen\output_json\文本对比结果_20260228_135825.json"
    with open(body_result_path, 'r', encoding='utf-8-sig') as f:
        file_content = f.read()

    # 2. 将读取到的内容传给解析器
    json_body = jsonParser.load_error_json_text(file_content)
    print(json_body)