...
def parse_json_content(raw_content: str) -> List[dict]:
    ...
    if content.strip().startswith('"'):
        try:
            ...
            print("[OK] 成功解包：检测到文件是序列化字符串，已完成解包。")
        except json.JSONDecodeError:
            print("[WARN] 尝试解包失败，将按原样处理。")
    ...
    if content.strip().startswith('```'):
        ...
        if start_idx != -1 and end_idx != -1:
            ...
            content = '\n'.join(json_lines)
            print("[OK] 移除 Markdown 代码块标记")

    # 3. 尝试直接解析 JSON（如果格式正确）
    try:
        data = json.loads(content)
        if isinstance(data, list):
            print(f"[OK] 直接解析成功：提取到 {len(data)} 个对象")
            # 排序
            data.sort(key=lambda x: int(x.get("错误编号", 99999)))
            return data
    except json.JSONDecodeError as e:
        print(f"[WARN] 直接解析失败 ({e.msg})，使用栈式提取...")
                ...
                if stack_depth == 0 and start_index != -1:
                    ...
                    try:
                        ...
                        try:
                            ...
                            objects.append(obj)
                            print(f"  [OK] 修复控制字符后成功解析错误 {obj.get('错误编号', '?')}")
                        except:
                            ...
                            try:
                                ...
                                obj = json.loads(fixed_str)
                                objects.append(obj)
                                print(f"  [OK] 手动修复控制字符后成功解析错误 {obj.get('错误编号', '?')}")
                            except Exception as final_error:
                                print(f"  [ERROR] 无法解析对象 (位置 {start_index}-{i}): {final_error}")
...
if __name__ == "__main__":
    ...
    if results:
        print(f"[OK] 解析成功！共获取 {len(results)} 个错误条目。")
        print(f"第一个错误: ID {results[0].get('错误编号')} - {results[0].get('错误类型')}")
    else:
        print("[ERROR] 依然没有提取到数据。请再次运行并告诉我报错信息。")
