...
def _replace_numbering_with_context(doc, old_value, new_value, reason,
    ...
    if best_auto and best_num_id is not None:
        # 自动编号：为这个段落单独创建新的编号定义，设置 startOverride
        # 不能直接改原 w:num 的 startOverride，那会影响所有使用同一 numId 的段落
        ilvl = best_ilvl or '0'
        try:
            numbering_part = doc.part.numbering_part
            numbering_xml = numbering_part.element

            # 找到原 w:num 的 abstractNumId
            ...
            # 创建新的 w:num 元素，引用同一个 abstractNum
            ...
            # 在新 num 上设置 lvlOverride + startOverride
            ...
            # 把目标段落的 numId 指向新的 num
            pPr = best_para._element.pPr
            numPr = pPr.numPr
            numPr.numId.set(_qn('w:val'), new_num_id)

            return True, f"编号上下文定位(自动编号 新numId={new_num_id} start={new_num_int}) [{region_desc}] (得分:{best_score:.2f}, 候选:{len(candidates)})"
...
def replace_and_revise_in_docx(
    ...
    if is_list_pattern(old_value):
        # 判断是"格式变更"还是"序号值错误"
        # 格式变更：如 i. → 1)（罗马→阿拉伯），需要改编号定义
        # 序号值错误：如 iv. → v.（同格式不同值），需要改单个段落
        ...
        if is_same_format:
            ...
            numbering_ok = False
            try:
                from 数值检查1.llm.llm_project.replace.numbering_replacer import replace_numbering_in_docx
    ...
    if found:
        ...
        for p in paragraph_iterator():
            ...
            if full_text == matched_text:
                ...
                # 尝试用原始值（未 strip）
                ok = _execute_replace(p, old_value_original, new_value, reason, revision_manager)
                if ok:
                    return True, f"{strategy} [{region_desc}]"
    ...
    # 定义各策略的正则模式和最低上下文相似度门槛
    strategies = [
        # (策略名, 正则模式, 最低相似度门槛, 最大匹配层级)
    ...
    for strategy_name, pattern, min_similarity, max_level in strategies:
        ...
        candidates = []  # [(score, paragraph, match_level)]
        ...
        if context and min_similarity > 0:
            ...
            if sim < min_similarity:
                print(f"    [{strategy_name}] 最佳候选相似度 {sim:.2f} < 门槛 {min_similarity}，跳过")
                continue

        # 检查2：唯一性安全网
        # 多处出现 + 无上下文 + 非精确锚点策略 → 拒绝替换
        if len(candidates) > 1 and not context and not anchor_text:
            print(f"    [{strategy_name}] old_value 出现 {len(candidates)} 处且无上下文，拒绝替换避免歧义")
            continue

        # 检查3：多处出现时，最佳和次佳得分差距太小 → 不确定，拒绝
        if len(candidates) > 1:
            second_score = candidates[1][0]
            if best_score > 0 and second_score / best_score > 0.95:
                # 两个候选得分几乎一样，无法区分
                print(f"    [{strategy_name}] 前两个候选得分接近 ({best_score:.3f} vs {second_score:.3f})，无法确定位置")
                continue
