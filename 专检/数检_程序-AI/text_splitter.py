...
def split_text(text: str, num_parts: int,
               ...
               buffer_chars: int = BUFFER_CHARS) -> Tuple[List[str], List[Tuple[int, int]]]:
    ...
    Returns:
        (chunks, index_ranges)
        chunks: 切分后的文本块列表
        index_ranges: 每块对应的段落索引范围列表 [(buf_start, buf_end), ...]
                      含缓冲区扩展后的实际范围，左闭右开
    """
    if num_parts <= 1 or not text:
        paragraphs = text.split('\n') if text else []
        return ([text] if text else []), [(0, len(paragraphs))]
    ...
    for i in range(len(boundaries) - 1):
        ...
        index_ranges.append((buf_start, buf_end))

    return chunks, index_ranges


def split_text_pair(orig_text: str, trans_text: str,
                    chunk_size: int = DEFAULT_CHUNK_SIZE,
                    buffer_chars: int = BUFFER_CHARS) -> List[Tuple[str, str, Tuple[int, int], Tuple[int, int]]]:
    ...
    Returns:
        [(orig_chunk, trans_chunk, orig_range, trans_range), ...]
        orig_range/trans_range: (buf_start, buf_end) 段落索引，左闭右开
    """
    if not orig_text or not trans_text:
        orig_paras = len(orig_text.split('\n')) if orig_text else 0
        trans_paras = len(trans_text.split('\n')) if trans_text else 0
        return [(orig_text or "", trans_text or "", (0, orig_paras), (0, trans_paras))]

    # 算分块数
    num_parts = auto_num_parts(orig_text, chunk_size)

    orig_paras_total = len(orig_text.split('\n'))
    trans_paras_total = len(trans_text.split('\n'))

    if num_parts <= 1:
        return [(orig_text, trans_text, (0, orig_paras_total), (0, trans_paras_total))]
    ...
    orig_chunks, orig_ranges = split_text(orig_text, num_parts, split_ratios=None, buffer_chars=buffer_chars)

    # 译文：按原文的比例映射切
    trans_chunks, trans_ranges = split_text(trans_text, num_parts, split_ratios=ratios, buffer_chars=buffer_chars)

    # 对齐块数（理论上应该相等，防御性处理）
    min_len = min(len(orig_chunks), len(trans_chunks))
    pairs = list(zip(orig_chunks[:min_len], trans_chunks[:min_len],
                     orig_ranges[:min_len], trans_ranges[:min_len]))

    # 如果有剩余，追加到最后一块
    if len(orig_chunks) > min_len:
        extra_orig = '\n'.join(orig_chunks[min_len:])
        last_o, last_t, last_or, last_tr = pairs[-1]
        pairs[-1] = (last_o + '\n' + extra_orig, last_t, last_or, last_tr)
    if len(trans_chunks) > min_len:
        extra_trans = '\n'.join(trans_chunks[min_len:])
        last_o, last_t, last_or, last_tr = pairs[-1]
        pairs[-1] = (last_o, last_t + '\n' + extra_trans, last_or, last_tr)
...
def get_overlap_text(prev_chunk: Tuple[str, str, Tuple[int, int], Tuple[int, int]],
                     curr_chunk: Tuple[str, str, Tuple[int, int], Tuple[int, int]]
                     ) -> Tuple[str, str]:
    """从相邻两个 chunk 中提取实际重叠的段落文本。

    通过比较段落索引范围的交集来确定重叠区域，
    比逐行字符串匹配更准确（不受 LLM 改写影响）。

    Args:
        prev_chunk: 上一块 (src_text, tgt_text, src_range, tgt_range)
        curr_chunk: 当前块 (src_text, tgt_text, src_range, tgt_range)

    Returns:
        (src_overlap_text, tgt_overlap_text)
        若无重叠则返回 ("", "")
    """
    _, _, prev_src_range, prev_tgt_range = prev_chunk
    _, _, curr_src_range, curr_tgt_range = curr_chunk

    def _extract_overlap(text: str, buf_start: int,
                         prev_range: Tuple[int, int], curr_range: Tuple[int, int]) -> str:
        overlap_start = max(prev_range[0], curr_range[0])
        overlap_end   = min(prev_range[1], curr_range[1])
        if overlap_start >= overlap_end:
            return ""
        paras = text.split('\n')
        # 绝对索引转为相对于当前块 buf_start 的局部索引
        local_start = overlap_start - buf_start
        local_end   = overlap_end   - buf_start
        local_start = max(0, local_start)
        local_end   = min(len(paras), local_end)
        if local_start >= local_end:
            return ""
        return '\n'.join(paras[local_start:local_end])

    prev_src_text, prev_tgt_text, _, _ = prev_chunk
    curr_src_text, curr_tgt_text, _, _ = curr_chunk

    # 用当前块的文本 + 当前块的 buf_start 做偏移修正
    src_overlap = _extract_overlap(curr_src_text, curr_src_range[0], prev_src_range, curr_src_range)
    tgt_overlap = _extract_overlap(curr_tgt_text, curr_tgt_range[0], prev_tgt_range, curr_tgt_range)

    return src_overlap, tgt_overlap
...
if __name__ == "__main__":
    ...
    src_paras_total = len(src_text.split('\n'))
    tgt_paras_total = len(tgt_text.split('\n'))
    print(f"📊 统计 -> 原文总字数: {_count_chars(src_text):,} 字 / {src_paras_total:,} 段(含空行) | 译文总词数: {_count_chars(tgt_text):,} 词 / {tgt_paras_total:,} 段(含空行)")
    chunks = split_text_pair(src_text, tgt_text)
    # 遍历 chunks 列表
    for i, chunk in enumerate(chunks, 1):
        src_part, tgt_part, src_range, tgt_range = chunk
        src_len = _count_chars(src_part)
        tgt_len = _count_chars(tgt_part)

        # 1. 打印块标题
        print(f"{'=' * 20} 第 {i:02d} 块 (Chunk {i}) {'=' * 20}")
        print("当前原文块:",src_part)
        print("当前译文块:",tgt_part)

        # 2. 打印重叠区间（从第 2 块开始，用索引范围精确提取重叠段落）
        if i > 1:
            src_overlap, tgt_overlap = get_overlap_text(chunks[i - 2], chunks[i - 1])

            if src_overlap or tgt_overlap:
                print(f"🔗 【承接自上一块的重叠内容】:")
                if src_overlap:
                    clean_src = src_overlap.replace('\n', ' ↵ ')
                    print(f"  原文({_count_chars(src_overlap)}字): {clean_src[:120]}{'...' if len(clean_src) > 120 else ''}")
                if tgt_overlap:
                    clean_tgt = tgt_overlap.replace('\n', ' ↵ ')
                    print(f"  译文({_count_chars(tgt_overlap)}词): {clean_tgt[:120]}{'...' if len(clean_tgt) > 120 else ''}")
                print("-" * 50)
            else:
                print("⚠️ [警告]: 未检测到重叠，可能存在断层！")

        # 3. 打印当前块的统计信息
        src_paras = len([p for p in src_part.split('\n') if p.strip()])
        tgt_paras = len([p for p in tgt_part.split('\n') if p.strip()])
        print(f"📊 统计 -> 原文: {src_len:,} 字 / 段落[{src_range[0]}~{src_range[1]-1}]({src_paras}段) | 译文: {tgt_len:,} 词 / 段落[{tgt_range[0]}~{tgt_range[1]-1}]({tgt_paras}段)")

        # 4. 打印正文内容（可以根据需要限制打印长度）
        # print(f"【原文全文】:\n{src_part}")
        # print(f"【译文全文】:\n{tgt_part}")

        # 5. 异常提醒
        if src_len > 0 and (tgt_len / src_len > 2 or tgt_len / src_len < 0.3):
            print("🚨 注意：该块原译文长度比例异常（差异超过2倍），请检查对齐。")

        # 6. 最后一块：打印末尾最后5段确认是否到底
        if i == len(chunks):
            src_lines = [p for p in src_part.split('\n') if p.strip()]
            tgt_lines = [p for p in tgt_part.split('\n') if p.strip()]
            print("📌 原文最后7段:")
            print('\n'.join(src_lines[-7:]))
            print("📌 译文最后7段:")
            print('\n'.join(tgt_lines[-7:]))
