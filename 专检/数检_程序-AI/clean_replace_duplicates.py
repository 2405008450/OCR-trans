...
def clean_suggestion(context: str, anchor: str, suggestion: str) -> str:
    """
    在写回之前，检测并剪裁 suggestion 与上下文前后窗口的重叠部分。
    只修改 suggestion，不修改 context，供 apply_revisions_from_ai_map 调用。

    Args:
        context:    AI 返回的译文上下文（用于检测重叠）
        anchor:     替换锚点（old_val）
        suggestion: AI 建议的新值（new_val）

    Returns:
        剪裁后的 suggestion；若锚点不在上下文中则直接返回原 suggestion。
    """
    ...
    # --- 前部去重：suggestion 开头和锚点前窗口结尾重叠 ---
    prefix_window = context[max(0, start_idx - len(suggestion)):start_idx]
    s = difflib.SequenceMatcher(None, prefix_window, actual_sug)
    match = s.find_longest_match(0, len(prefix_window), 0, len(actual_sug))
    if match.size > 0 and (match.a + match.size == len(prefix_window)) and (match.b == 0):
        actual_sug = actual_sug[match.size:]

    # --- 后部去重：suggestion 结尾和锚点后窗口开头重叠 ---
    ...
    return actual_sug or suggestion  # 剪裁后为空时退回原值，避免把 new_val 清空
...
if __name__ == '__main__':
    ...
    print(res3)
    # --- 测试场景 4：防止文本重复(末尾，开头，中间) ---
    ctx3 = "6.1.5 Changes in the fair value of investments in other debt obligations"
    anc3 = "6.1.5"
    sug3 = "5 Changes in the fair value of investments in other debt obligations"
    res3, _ = ultra_safe_replace_v2_debug(ctx3, anc3, sug3)
    print(res3)
