...
用法：
    python test_quick_replace.py "译文.docx" "output/align_body_flat_errors.json"
    python test_quick_replace.py "译文.xlsx" "output/align_body_flat_errors.json"
    python test_quick_replace.py "译文.pptx" "output/align_body_flat_errors.json"
    python test_quick_replace.py "译文.pdf"  "output/align_body_flat_errors.json"
"""
...
def _run_docx(backup_path: str, tasks: list) -> list:
    ...
    results = []
    for error_no, old_val, new_val, reason, context, anchor in tasks:
...
def quick_test(doc_path: str, errors_json: str = None, test_cases: list = None):
    ...
    for seq_idx, err in enumerate(test_cases):
        ...
        tasks.append((seq_idx, error_no, old_val, cleaned, reason, context, anchor))
    ...
    tasks_flat = [(error_no, old_val, new_val, reason, context, anchor)
                  for seq_idx, error_no, old_val, new_val, reason, context, anchor in tasks]
